<template>
  ...
  .image-container {
    margin-right: 20px;
    width: 40%; /* 이미지 컨테이너의 고정된 너비 */
    aspect-ratio: 1/1;
    overflow: hidden; /* 이미지가 넘칠 경우 가립니다. */
  }
  
  .image-container img {
    width: 100%; /* 이미지를 컨테이너에 꽉 차게 표시합니다. */
    height: 100%; /* 이미지를 컨테이너에 꽉 차게 표시합니다. */
    object-fit: cover; /* 이미지를 자르지 않고 비율을 유지하면서 채웁니다. */
  }
  ...
  .content-container {
    ...
    justify-content: flex-start; /* 요소들을 위쪽으로 정렬합니다. */
  }
  ...
  .pubDate {
    font-size: 14px; /* 발행일의 글자 크기 */
    color: #888; /* 회색으로 표시합니다. */
    margin-bottom: 5px;
  }
  ...
  .textContent {
    ...
    width: 100%;
    line-height: 1.5; /* 줄간격 */
    overflow: hidden; /* 텍스트가 넘칠 경우 가립니다. */
    text-overflow: ellipsis; /* 텍스트가 넘칠 경우 말줄임표를 표시합니다. */
    display: -webkit-box;
